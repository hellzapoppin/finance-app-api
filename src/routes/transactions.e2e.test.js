...
describe('Transaction Routes E2E Tests', () => {
    ...
    it('GET /api/transaction/me?from=2025-01-01&to=2025-01-31 should return 200 when fetching transactions successfully', async () => {
        const { body: createdUser } = await request(app)
            .post('/api/users')
            .send({ ...user, id: undefined })
    ...
    it('PATCH /api/transactions/me/:transactionId should return 404 when updating a non-existing transaction', async () => {
        ...
        const response = await request(app)
            .post(`/api/transactions/me/${transaction.id}`)
            .set('Authorization', `Bearer ${createdUser.tokens.accessToken}`)
            .send({ amount: 100, type: TransactionType.INVESTMENT })
