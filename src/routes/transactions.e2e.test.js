...
describe('Transaction Routes E2E Tests', () => {
    ...
    it('GET /api/transaction?user_id should return 200 when fetching transactions successfully', async () => {
        const { body: createdUser } = await request(app)
            .post('/api/users')
            .send({ ...user, id: undefined })
    ...
    it('PATCH /api/transactions/:transactionId should return 404 when updating a non-existing transaction', async () => {
        const response = await request(app)
            .post(`/api/transactions/${transaction.id}`)
            .send({ amount: 100, type: TransactionType.INVESTMENT })
    ...
    it('GET /api/transaction?user_id should return 404 when fetching transaction from a non-existing user', async () => {
        const response = await request(app).get(
            `/api/transactions?user_id=${user.id}`,
        )
